...
export default function Chat() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 pt-14 pb-32 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto"></div>
          <p className="mt-4 text-blue-600">Đang tải tin nhắn...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 pt-14 pb-32 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-500">Lỗi tải tin nhắn: {error.message}</p>
          <button 
        ...
        <div className="space-y-4">
          {messages.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-sm p-8 text-center border border-blue-200">
              <p className="text-blue-600 text-lg">Không có tin nhắn nào. Hãy bắt đầu cuộc trò chuyện!</p>
            </div>
          ) : (
            messages.map((msg) => (
        ...
        <div className="max-w-2xl mx-auto">
          <form onSubmit={handleSubmit(onSubmit)} className="flex gap-2">
            <div className="flex-1">
              ...
              {sendError && (
                <p className="text-red-500 text-sm mt-1 ml-1">Gửi tin nhắn thất bại</p>
              )}
            </div>
